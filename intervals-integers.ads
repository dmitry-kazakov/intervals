...
--
--  This package provides interval arithmetic for  integer  numbers.  No
--  overflow  checks made explicitly. The package is generic, the formal
--  parameter is the type used for the bounds:
--
...
--  result.  For  integer  arithmetic,  in  which  only  division is not
--  precise, it means that for instance, [1,1]/2 = [0,1], while  1/2  is
--  0. Equality is implemented as in set theory, because an  alternative
--  definition ([a,b]=[c,d] <=> for any x from [a,b], y from [c,d]; x=y)
--  is rather useless. But other relational operators are implemented in
--  this  sense. If @ is a relational operator defined on numbers, then:
--  [a,b] @ [c,d] is True if for any x from [a,b], y from [c,d]; x @  y.
--  It  is  False  if for any x from [a,b], y from [c,d]; not x@y. It is
--  Uncertain otherwise.
--
generic
...
-- result interval covers all possible outcomes.  Therefore  [3,3]/[2,2]
-- is [1,2] and not 1 in contrast to 3/2 = 1.
--
-- Returns :
--
--    Left / Right
